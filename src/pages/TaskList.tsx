...
interface Props {
  ...
  users: User[];
  currentUser: User;
  onAddUpdate: (id: string, content: string, status?: TaskStatus, evidence?: { data: string, name: string }, mentions?: string[]) => void;
  onCloseTask: (id: string) => void;
  onUpdateTaskDate: (id: string, newDate: string, reason: string) => void;
  onUpdateRACI: (id: string, newRaci: RACIMatrix) => void;
}

const TaskList: React.FC<Props> = ({ 
  tasks, users, currentUser, onAddUpdate, onCloseTask, onUpdateTaskDate, onUpdateRACI 
}) => {
  ...
  const submitDateChange = () => {
    ...
    if (selectedTask) {
      onUpdateTaskDate(selectedTask.id, reqDate, reqReason);
      setReqDate('');
      setReqReason('');
        ...
        {selectedTask ? (
          <>
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
               <div className="flex-[7] overflow-y-auto p-6 lg:p-8 space-y-6 custom-scrollbar bg-white lg:border-r border-slate-100">
               ...
               <div className="flex-[3] bg-slate-50/50 p-6 space-y-8 overflow-y-auto custom-scrollbar">
              ...
              <div className="p-4 lg:p-6 border-t border-slate-100 bg-white relative">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <button onClick={() => { 
                      if (updateText.trim()) { 
                        onAddUpdate(selectedTask.id, updateText, (targetStatus as TaskStatus) || undefined, evidence || undefined, mentions); 
                        setUpdateText(''); setTargetStatus(''); setMentions([]); setEvidence(null);
                      } 
                    }} className="w-full sm:w-auto bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-xs hover:bg-blue-800 hover:shadow-lg transition-all active:scale-95 shadow-lg">Kirim Update</button>
                  </div>
                </div>
              </div>
            )}
